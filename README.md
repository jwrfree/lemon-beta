...
Lemon Coach also maintains `public.user_financial_profile` for longer-term coaching memory. After a substantive session closes, the client fires a background `/api/chat/update-profile` request; the server summarizes the session into `spending_patterns` and `coaching_notes`, upserts that profile, and injects it back into later chat prompts alongside `memory_summary`.
...
Within the Phase 4 finance features (`transactions`, `budgets`, `wallets`, `insights`, `ai-chat`), hooks/providers now call feature services instead of reaching into `supabase.from(...)` directly, so new RPC-backed behavior can be added without bypassing business rules.
...
DS-3 also adds a lightweight `PageTransition` wrapper around the main App Router content shell, using a 150ms fade-and-rise animation based on `opacity` and `transform` only so route changes feel smoother on mobile without interfering with drawer or modal motion.
