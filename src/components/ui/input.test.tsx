...
describe('Input', () => {
  it('uses text-body-lg by default to avoid iOS zoom on focus', () => {
    render(<Input aria-label="Email" />);

    const input = screen.getByRole('textbox', { name: 'Email' });
    expect(input.className).toContain('text-body-lg');
    expect(input.className).not.toContain('text-body-md');
  });
  ...
  it('keeps the large size variant at a 16px floor on mobile', () => {
    ...
    expect(input.className).toContain('text-body-lg');
    expect(input.className).not.toContain('md:text-body-lg');
  });
});
