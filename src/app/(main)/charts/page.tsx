...
import { ChevronLeft, ArrowUpRight, ArrowDownLeft, Scale, TrendingDown, Landmark, ReceiptText, Tags, Briefcase } from 'lucide-react';
...
const IncomeSummaryCard = () => {
    ...
    const { icon: CategoryIcon } = summaryData.biggestSource ? categoryDetails(summaryData.biggestSource.name) : { icon: Briefcase };
    ...
    return (
        <Card>
            ...
            <CardContent className="flex flex-col items-center justify-center gap-4 text-center">
                <p className="text-4xl font-bold text-green-600">{formatCurrency(summaryData.totalIncome)}</p>
                <div className="grid grid-cols-2 gap-4 w-full pt-2">
                    <div className="flex flex-col gap-1 items-center text-center p-2 rounded-lg bg-muted/50">
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            <Tags className="h-4 w-4" />
                        ...
                        <div className="flex items-center justify-center gap-1.5 font-semibold text-sm mt-1">
                            {summaryData.biggestSource && <CategoryIcon className="h-4 w-4 flex-shrink-0" />}
                            <span className="truncate">{summaryData.biggestSource?.name || '-'}</span>
                        </div>
                    </div>
                        <div className="flex flex-col gap-1 items-center text-center p-2 rounded-lg bg-muted/50">
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
...
const CategoryAnalysis = ({ type }: { type: 'expense' | 'income' }) => {
    ...
    if (chartData.length === 0) {
        return <PlaceholderContent label={`Analisis ${type === 'expense' ? 'Pengeluaran' : 'Pemasukan'}`} icon={type === 'expense' ? ArrowDownLeft : ArrowUpRight} text={`Data ${type}-mu belum cukup untuk dianalisis. Mulai catat transaksi yuk!`} />;
    }
