...
const PaymentItem = ({ payment, direction }: { payment: DebtPayment; direction: string }) => {
 ...
 <div>
 <p className="text-body-md font-medium">{formatCurrency(payment.amount)}</p>
 {paymentDate && (
 ...
 {payment.notes && <p className="text-label-md text-muted-foreground mt-1">{payment.notes}</p>}
 </div>
...
export default function DebtDetailPage() {
 ...
 <div className="flex-1 flex items-center justify-center text-muted-foreground text-body-md">
 ...
 <Card>
 <CardHeader>
 <CardTitle className="text-body-lg ">Ringkasan</CardTitle>
 </CardHeader>
 ...
 <span className="">{formatCurrency(outstanding)}</span>
 </div>
 {startDate && (
 <div className="flex items-center justify-between">
 ...
 <p className="text-body-md mt-1 whitespace-pre-wrap">{debt.notes}</p>
 </div>
 )}
 </CardContent>
 </Card>
 ...
 <CardTitle className="text-body-lg ">Riwayat Pembayaran</CardTitle>
 <Button
 size="sm"
 onClick={() => {
 ...
 {payments.length === 0 ? (
 <p className="text-body-md text-muted-foreground py-4 text-center">
