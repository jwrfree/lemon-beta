...
export default function RemindersPage() {
 ...
 key={key} 
 value={key} 
 className="rounded-full border bg-background data-[state=active]:bg-primary data-[state=active]:text-primary-foreground h-7 text-label-md px-3"
 >
 {label}
 </TabsTrigger>
 ))}
 ...
 <TabsTrigger value="week"className="h-full rounded-md font-medium text-label-md transition-all data-[state=active]:bg-background data-[state=active]:text-foreground">Minggu ini</TabsTrigger>
 <TabsTrigger value="30"className="h-full rounded-md font-medium text-label-md transition-all data-[state=active]:bg-background data-[state=active]:text-foreground">30 hari</TabsTrigger>
 </TabsList>
 </Tabs>
 <Button
 size="icon"
 ...
 <h2 className="text-label text-muted-foreground px-1">{dateLabel}</h2>
 {items.map(reminder => {
 const status = getReminderStatus(reminder);
 const dueDate = reminder.dueDate ? parseISO(reminder.dueDate) : null;
 ...
 : undefined;
 return (
 <Card key={reminder.id} className="p-5 bg-card border-none shadow-md rounded-card-glass">
 ...
 <h2 className="text-title-sm">{reminder.title}</h2>
 {renderStatusBadge(status)}
 </div>
 
 <div className="flex flex-wrap gap-x-4 gap-y-2">
 {dueDate && (
 <div className="text-label-md text-muted-foreground/60 flex items-center gap-1.5 ">
 <CalendarClock className="h-3.5 w-3.5"/>
 {format(dueDate, 'd MMM yyyy', { locale: dateFnsLocaleId })}
 </div>
 )}
 {reminder.amount ? (
 <div className="text-label-md text-primary flex items-center gap-1.5 bg-primary/5 px-2 py-0.5 rounded-full">
 ...
 {linkedDebt && (
 <p className="text-label-md text-muted-foreground/40 ">
 Terkait: {linkedDebt.title}
 </p>
 )}
 
 {reminder.notes && (
 <p className="text-label-md font-medium text-muted-foreground leading-relaxed italic border-l-2 border-muted pl-3">{reminder.notes}</p>
 )}

 <div className="flex items-center gap-2 pt-2">
 {status !== 'completed'&& (
 <Button size="sm"variant="secondary"className="h-9 px-4 rounded-full text-label-md gap-2 bg-primary/10 text-primary hover:bg-primary/20"onClick={() => handleComplete(reminder)}>
 ...
 <>
 <DropdownMenuItem className="text-label-md p-3"onClick={() => handleSnooze(reminder, 1)}>Tunda 1 hari</DropdownMenuItem>
 <DropdownMenuItem className="text-label-md p-3"onClick={() => handleSnooze(reminder, 3)}>Tunda 3 hari</DropdownMenuItem>
 </>
 )}
 <DropdownMenuItem className="text-label-md p-3"onClick={() => { setReminderToEdit(reminder); setIsReminderModalOpen(true); }}>
 Edit
 </DropdownMenuItem>
 <DropdownMenuItem className="text-label-md p-3 text-destructive"onClick={() => handleDelete(reminder)}>
 Hapus
 </DropdownMenuItem>
 </DropdownMenuContent>
 </DropdownMenu>
 ...
 <h2 className="text-label text-muted-foreground">{dateLabel}</h2>
 </div>
 ...
 {reminder.amount ? (
 <p className="text-body-md font-medium">{formatCurrency(reminder.amount)}</p>
 ) : null}
 {reminder.notes && (
 <p className="text-label-md text-muted-foreground">{reminder.notes}</p>
 )}
 </div>
 <div className="flex items-center gap-2">
 {status !== 'completed'&& (
 ...
 <CardTitle className="flex items-center gap-2 text-body-lg font-medium">
 ...
 <p className="text-label-md text-destructive/80">Terlambat</p>
 <p className="text-title-lg font-medium">
 {reminders.filter(r => getReminderStatus(r) === 'overdue').length}
 </p>
 </div>
 <div className="rounded-lg bg-primary/10 text-primary p-3">
 <p className="text-label-md text-primary/80">Segera</p>
 <p className="text-title-lg font-medium">
 {reminders.filter(r => {
 if (!r.dueDate || getReminderStatus(r) === 'completed') return false;
 const diff = differenceInCalendarDays(parseISO(r.dueDate), new Date());
 ...
 <CardTitle className="text-body-md font-medium">Aksi Cepat</CardTitle>
 </CardHeader>
 ...
 <Button 
 className="flex-1 h-14 rounded-full text-label-md shadow-lg shadow-primary/20"
