    ...
    <Wrapper {...pageProps}>
      <MotionWrapper {...contentProps}>
        ...
        <div className="p-5 pb-6 overflow-y-auto">
          <p className="text-body-md text-muted-foreground mb-4">Masuk menggunakan email yang sudah terdaftar untuk membuka dashboard Lemon.</p>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleLogin)} className={layout.stack}>
              <FormField
              ...
              <div className="flex items-center justify-between">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex items-center gap-2 space-y-0">
                      ...
                      <FormLabel htmlFor="remember" className="text-body-md font-normal cursor-pointer">
                        Ingat saya
                      </FormLabel>
                    </FormItem>
                  )}
                ...
                <Button
                  variant="link"
                  type="button"
                  className="p-0 h-auto text-body-md"
                  onClick={() => {
                    setAuthError(null);
              ...
              <div className="mt-8 space-y-4">
                <Button
                  type="submit"
                  size="lg"
                  className={cn("w-full transition-all active:scale-[0.98]", layout.primaryCTA)}
                  disabled={isSubmitting}
                >
                  {isSubmitting && <LoaderCircle className="mr-2 h-5 w-5 animate-spin"/>}
                  {isSubmitting ? 'Masuk...': 'Masuk'}
                </Button>

                <div className="relative">
                  <Separator className="bg-border/40"/>
                  <span className="absolute inset-x-0 -top-2 mx-auto w-max bg-background px-3 text-label-md text-muted-foreground/60">atau lanjutkan</span>
                </div>
          ...
          <p className="text-body-md text-muted-foreground mt-8 text-center">
            Belum punya akun? {''}
            <Button
              variant="link"
              onClick={() => {
