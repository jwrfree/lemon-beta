...
export const ExpenseShortTermTrend = ({ transactions, isLoading }: { transactions: Transaction[], isLoading?: boolean }) => {
 ...
 key={option} 
 value={option}
 className="h-full rounded-full px-3 text-label-md font-medium text-label transition-all data-[state=active]:bg-white data-[state=active]:text-slate-950"
 >
 {option} Hari
 </TabsTrigger>
 ))}
 ...
 key={option.value}
 value={option.value}
 className="h-full flex items-center gap-1 rounded-full px-3 text-label-md font-medium text-label transition-all data-[state=active]:bg-white data-[state=active]:text-slate-950"
 >
 ...
 <p className="text-label-md sm:text-label-md font-medium text-label text-muted-foreground mb-1">Total</p>
 <p className="text-body-lg sm:text-title-lg font-medium text-foreground tabular-nums">{formatCurrency(totalSpent)}</p>
 <p className={cn("text-label-md sm:text-label-md font-medium mt-1", delta > 0 ? "text-destructive": "text-teal-600")}>
 {percentChange ? (percentChange > 0 ? '+': '') + percentChange.toFixed(1) + '%': '—'}
 </p>
 </div>
 <div className="rounded-lg bg-muted/50 p-3 sm:p-4 border-none">
 <p className="text-label-md sm:text-label-md font-medium text-label text-muted-foreground mb-1">Rata-rata</p>
 <p className="text-body-lg sm:text-title-lg font-medium text-foreground tabular-nums">{formatCurrency(average)}</p>
 <p className="text-label-md sm:text-label-md font-medium text-muted-foreground mt-1 text-label">PER HARI</p>
 </div>
 <div className="rounded-lg bg-muted/50 p-3 sm:p-4 border-none col-span-2 sm:col-span-1">
 <p className="text-label-md sm:text-label-md font-medium text-label text-muted-foreground mb-1">Puncak</p>
 {peakDay ? (
 <div className="flex items-center justify-between sm:block">
 <p className="text-body-lg sm:text-title-lg font-medium text-foreground tabular-nums">{formatCurrency(peakDay.total)}</p>
 <p className="text-label-md sm:text-label-md font-medium text-muted-foreground sm:mt-1 text-label">{peakDay.shortLabel}</p>
 </div>
 ) : (
 <p className="text-body-md text-muted-foreground">None</p>
 )}
 </div>
 </div>
