...
const getDebtDueStatus = (debt: Debt) => {
    ...
    if (diff < 0) {
        return (
            <span className="text-label flex w-fit items-center gap-1.5 rounded-full bg-destructive/10 px-2 py-0.5 font-semibold uppercase tracking-widest text-destructive">
    ...
    if (diff === 0) {
        return (
            <span className="text-label flex w-fit items-center gap-1.5 rounded-full bg-warning/10 px-2 py-0.5 font-semibold uppercase tracking-widest text-warning">
                <CalendarDots className="h-3 w-3" weight="regular" />
                Hari ini
            </span>
        );
    ...
    return (
        <span className="text-label flex w-fit items-center gap-1.5 rounded-full bg-muted px-2 py-0.5 font-semibold uppercase tracking-widest text-muted-foreground/45">
            <CalendarDots className="h-3 w-3" weight="regular" />
            {diff} hari lagi
        </span>
    );
                    ...
                    <div className="relative z-10 flex flex-col gap-6">
                        <div className="flex items-start justify-between">
                            <div className="space-y-1.5">
                                <h3 className="line-clamp-1 text-base font-semibold tracking-tight text-foreground">{debt.title}</h3>
                                <div className="flex items-center gap-2">
                                    <div className="label-xs flex items-center gap-1.5 lowercase !text-muted-foreground/35">
                                        <span>{isOwed ? 'kepada' : 'dari'}</span>
                                        <span className="font-semibold text-muted-foreground/60">{debt.counterparty}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-end justify-between">
                                <div className="space-y-1">
                                    ...
                                    <p
                                        className={cn(
                                            'text-2xl font-medium leading-tight tracking-tighter tabular-nums',
                            ...
                            <div className="space-y-2.5">
                                <div className="flex items-end justify-between">
                                    ...
                                    <span className="text-label font-bold tabular-nums text-muted-foreground/45">
            ...
            <div className="space-y-6 px-1">
                <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                        ...
                        <p className="text-2xl font-medium leading-tight tracking-tighter tabular-nums text-foreground">
                ...
                <div className="flex items-center gap-2">
                            ...
                            <TabsList className="h-10 w-fit rounded-full border-none bg-muted/50 p-1">
                                {Object.entries(filterLabels).map(([value, label]) => (
                                    <TabsTrigger
                                        key={value}
                                        value={value}
                                        className="h-full rounded-full px-4 text-label font-semibold uppercase tracking-widest transition-all data-[state=active]:bg-white data-[state=active]:text-foreground data-[state=active]:shadow-sm dark:data-[state=active]:bg-zinc-800"
                                    >
                                        {label}
                                    </TabsTrigger>
                                ))}
                    ...
                    <Select value={sortBy} onValueChange={setSortBy}>
                        ...
                        <SelectContent align="end" className="rounded-[24px] border-none bg-popover/95 shadow-xl backdrop-blur-xl">
                            ...
                            <SelectItem value="due_soon" className="p-3 text-label font-semibold uppercase tracking-widest">
                                Jatuh Tempo
                            </SelectItem>
                            <SelectItem value="amount_desc" className="p-3 text-label font-semibold uppercase tracking-widest">
                                Nominal Tertinggi
                            </SelectItem>
                            <SelectItem value="amount_asc" className="p-3 text-label font-semibold uppercase tracking-widest">
                                Nominal Terendah
                            </SelectItem>
                        </SelectContent>
                    </Select>
