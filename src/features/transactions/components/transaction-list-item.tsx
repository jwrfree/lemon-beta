...
const TransactionListItemContent = ({
 ...
 const amountColor = isExpense ? 'text-foreground': 'text-success ';
 ...
 <div className="font-medium text-foreground text-body-md leading-snug tracking-tight truncate">
 ...
 <div
 className={cn(
 "text-body-md font-medium tracking-tighter tabular-nums",
 amountColor,
 !isBalanceVisible && 'blur-sm transition-all duration-300',
 isExpense && transaction.amount >= 1000000 && "text-body-lg"
 )}
 ...
 <span className="text-label bg-destructive/10 text-destructive px-1.5 py-0.5 rounded-full">High</span>
 )}
 </div>
 </div>
 );
