      ...
      <div className="space-y-3">
          ...
          {activeEditor === 'amount' ? (
            ...
            <>
              <button
                ...
                className={cn(
                  "mt-2 flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left transition-all active:scale-[0.98] shadow-sm",
                  amountNumber > 0
                    ? type === 'income'
                      ? "bg-secondary text-primary"
                      : "bg-card text-foreground"
                    : "animate-pulse bg-warning/10 text-warning",
                  errors.amount && "border-destructive border animate-shake"
                )}
              >
                <span className="min-w-0 flex-1 text-display-md tracking-tight">
                  {amountNumber > 0 ? formatCurrency(amountNumber) : 'Isi nominal'}
                </span>
                <PencilSimple size={18} className="text-muted-foreground/40" weight="regular" />
              </button>
              {errors.amount && <p className="mt-1.5 px-1 text-label-md text-destructive font-medium">{errors.amount.message as string}</p>}
            </>
          )}
        </div>
        ...
        <div className={cn(sectionCardClass, "space-y-2")}>
          ...
          <button
            ...
            className={cn(
              "flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left transition-all active:scale-[0.98] shadow-sm",
              categoryObj ? "bg-card text-foreground" : cn("animate-pulse", missingFieldClass),
              errors.category && "border-destructive border animate-shake"
            )}
          >
              ...
              {categoryObj ? (
                <>
                  ...
                  <div className="min-w-0">
                    <p className="line-clamp-1 text-body-md text-foreground">{categoryObj.name}</p>
                    {subCategoryName && (
                      <p className="line-clamp-1 text-label-md font-medium text-muted-foreground">{subCategoryName}</p>
                    )}
                  </div>
                </>
              ) : (
                <span className="text-body-md font-medium text-inherit">Pilih kategori</span>
              )}
            </div>
            <PencilSimple size={18} className="text-muted-foreground/40" weight="regular" />
          </button>
          {errors.category && <p className="mt-1.5 px-1 text-label-md text-destructive font-medium">{errors.category.message as string}</p>}
        </div>
          ...
          {activeEditor === 'description' ? (
            <div className="flex items-center gap-2 rounded-xl bg-muted p-3">
              <Input
                ...
                className="h-12 flex-1 rounded-2xl text-body-lg font-medium"
                autoFocus
            ...
            >
              <span className="line-clamp-1 min-w-0 flex-1 text-body-md font-medium">
          ...
          <div className="flex flex-wrap gap-2.5">
            ...
            <div className="flex flex-col gap-1.5">
              <button
                ...
                className={cn(
                  ...
                  walletObj ? "bg-card text-foreground shadow-sm" : cn("animate-pulse", missingFieldClass),
                  errors.walletId && "border-destructive border animate-shake"
                )}
              >
                <WalletIcon size={16} weight="regular" className="text-muted-foreground/60" />
                <span>{walletObj ? walletObj.name : 'Pilih dompet'}</span>
              </button>
              {errors.walletId && <p className="px-1 text-label-md text-destructive font-medium">{errors.walletId.message as string}</p>}
            </div>
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button
                  type="button"
                  className={cn(fieldButtonBase, "bg-card text-foreground shadow-sm")}
                >
                ...
                <div className="space-y-3">
                  ...
                  <div className="hide-scrollbar flex snap-x gap-2 overflow-x-auto pb-2">
                    {wallets.map((wallet) => (
                      ...
                      >
                        <span className="line-clamp-1 text-body-md ">{wallet.name}</span>
