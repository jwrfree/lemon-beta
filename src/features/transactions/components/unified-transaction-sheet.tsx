...
export const UnifiedTransactionSheet = ({
 ...
 const [imageUploadRequestKey, setImageUploadRequestKey] = useState(0);
 ...
 const handleQuickStartSuggestion = (suggestion: QuickStartSuggestion) => {
 ...
 if (suggestion.action === 'scan-receipt') {
 setImageUploadRequestKey((current) => current + 1);
 return;
 }

 setMagicValue(suggestion.inputText);
 ...
 onSuggestionClick={handleQuickStartSuggestion}
 />
 </motion.div>
 )}
 </AnimatePresence>
 ...
 <p className="text-body-md leading-relaxed text-warning-foreground">
 {clarificationQuestion}
 </p>
 </div>
 </div>
 ...
 <AlertDialogDescription className="text-body-md leading-relaxed text-muted-foreground/80">
