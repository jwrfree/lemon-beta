...
export type TransactionMutationClient = Pick<SupabaseClient, 'from' | 'rpc'>;
export type ExpenseIncomeTransactionValues = Extract<
...
export const mapTransactionRowToUnifiedValues = (
    transaction: TransactionRow,
): ExpenseIncomeTransactionValues => ({
    ...
    isNeed: transaction.is_need ?? true,
});

export const getTransactionRowById = async (
    client: TransactionMutationClient,
    userId: string,
    ...
    try {
        ...
        console.error('[TransactionService] Fetch Transaction Error:', err);
        return {
            data: null,
...
export const createTransactionWithClient = async (
    client: TransactionMutationClient,
    userId: string,
    ...
    try {
        ...
        if (data.type === 'transfer') {
            const { error } = await client.rpc('create_transfer_v1', {
                p_user_id: userId,
        ...
        const { data: result, error } = await client.rpc('create_transaction_v1', {
            p_user_id: userId,
...
export const updateTransactionWithClient = async (
    client: TransactionMutationClient,
    userId: string,
    transactionId: string,
    data: UnifiedTransactionFormValues,
...
export const deleteTransactionWithClient = async (
    client: TransactionMutationClient,
    userId: string,
    transactionId: string,
): Promise<ServiceResult<boolean>> => {
    try {
        const { error } = await client.rpc('delete_transaction_v1', {
            p_transaction_id: transactionId,
            p_user_id: userId,
        });
...
class TransactionService {
    ...
    async createTransaction(userId: string, data: UnifiedTransactionFormValues): Promise<ServiceResult<string>> {
        return createTransactionWithClient(this.supabase, userId, data);
    }
    ...
    async updateTransaction(
        ...
        return updateTransactionWithClient(this.supabase, userId, transactionId, data);
    }
    ...
    async deleteTransaction(userId: string, transactionId: string): Promise<ServiceResult<boolean>> {
        return deleteTransactionWithClient(this.supabase, userId, transactionId);
    }
