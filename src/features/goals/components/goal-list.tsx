...
export const GoalList = ({ goals }: { goals: Goal[] }) => {
 ...
 <CardTitle className="text-title-lg tracking-tight truncate">{goal.name}</CardTitle>
 {timeLeftText && (
 ...
 <span className="text-display-md tracking-tighter tabular-nums text-white">{formatCurrency(goal.currentAmount)}</span>
 </div>
 <div className="flex flex-col items-end">
 <span className="text-label text-white/40 mb-1.5 text-right">Target Goal</span>
 <span className="text-body-md tracking-tight text-white/70">{formatCurrency(goal.targetAmount)}</span>
 </div>
 </div>
