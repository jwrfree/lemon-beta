...
export const RemindersDashboard = ({ transactions }: RemindersDashboardProps) => {
  ...
  // Uses a fixed date matching the simulated environment time for first pass
  const now = React.useMemo(() => {
    if (!isMounted) return startOfDay(new Date(2026, 3, 5));
    return new Date();
  }, [isMounted]);
  ...
  const subscriptionSummary = useMemo(() => analyzeSubscriptions(transactions, now), [transactions, now]);
  const [aiInsight, setAiInsight] = useState<string | null>(null);
  const [isAiLoading, setIsAiLoading] = useState(false);

  useEffect(() => {
    const fetchAiInsight = async () => {
      if (isMounted && subscriptionSummary.activeSubscriptions > 0 && !aiInsight && !isAiLoading) {
        setIsAiLoading(true);
        try {
          const text = await auditSubscriptionsFlow(subscriptionSummary);
          setAiInsight(text);
        } catch (error) {
          console.error('Failed to fetch AI subscription audit:', error);
          // Gracefully fail by leaving aiInsight as null, which hides the UI
        } finally {
    ...
    fetchAiInsight();
  }, [isMounted, subscriptionSummary.activeSubscriptions, subscriptionSummary, aiInsight, isAiLoading]);

  const [activeTab, setActiveTab] = useState('upcoming');
  const [search, setSearch] = useState('');
  const [range, setRange] = useState<'week' | '30'>('week');

  const getReminderStatusInternal = React.useCallback((reminder: Reminder, referenceDate: Date) => {
    if (reminder.status === 'completed') return 'completed';
    const dueDate = reminder.dueDate ? parseISO(reminder.dueDate) : null;
    if (!dueDate) return 'upcoming';
    if (isBefore(dueDate, referenceDate) && reminder.status !== 'snoozed') {
      return 'overdue';
    }
    if (reminder.status === 'snoozed') {
      return isBefore(dueDate, referenceDate) ? 'overdue' : 'snoozed';
    }
    return 'upcoming';
  }, []);
  ...
  const filteredReminders = useMemo(() => {
    ...
    return sortedReminders.filter((reminder: Reminder) => {
      const status = getReminderStatusInternal(reminder, now);
      ...
      const withinRange = reminder.dueDate
        ? (() => {
          const diff = differenceInCalendarDays(parseISO(reminder.dueDate), today);
          return diff >= -1 && diff <= forwardDays;
        })()
        : true;
      return matchesStatus && matchesSearch && withinRange;
    });
  }, [sortedReminders, activeTab, search, range, today, now, getReminderStatusInternal]);
  ...
  const overdueRemindersCount = useMemo(() => reminders.filter(r => getReminderStatusInternal(r, now) === 'overdue').length, [reminders, now, getReminderStatusInternal]);
  const upcomingRemindersCount = useMemo(() => reminders.filter(r => {
    if (!r.dueDate || getReminderStatusInternal(r, now) === 'completed') return false;
    const diff = differenceInCalendarDays(parseISO(r.dueDate), now);
    return diff >= 0 && diff <= 7;
  }).length, [reminders, now, getReminderStatusInternal]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500 app-page-body-padding">
      ...
      {isMounted && (aiInsight || isAiLoading) && (
        ...
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="h-11 w-11 rounded-full bg-muted/30 hover:bg-muted/50 border-none">
          ...
          groupedByDate.map(([dateKey, items]) => {
            ...
            const dateLabel = isNoDate
              ...
              : isSameDay(parseISO(dateKey), today)
                ? 'Hari ini'
                : isSameDay(parseISO(dateKey), addDays(today, 1))
                  ? 'Besok'
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 space-y-3 min-w-0">
                          ...
                          <div className="flex items-center gap-2 pt-1">
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-11 w-11 rounded-full hover:bg-muted" aria-label="Opsi pengingat">
