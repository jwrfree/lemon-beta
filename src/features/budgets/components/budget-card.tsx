...
export const BudgetCard = ({ budget, transactions }: { budget: Budget, transactions: Transaction[] }) => {
 ...
 <div className={cn("p-3 rounded-card-icon backdrop-blur-xl transition-all", currentHealth.bg)}>
 <CategoryIcon className={cn("h-6 w-6 text-white")} strokeWidth={2.5} />
 </div>
 <div className="space-y-0.5">
 <h3 className="text-title-lg tracking-tight leading-tight group-hover:underline decoration-white/30">{budget.name}</h3>
 <div className="flex items-center gap-2">
 <span className={cn("text-label text-white/50")}>
 ...
 <div className="flex justify-between items-end text-label text-white/40">
 ...
 <div className="space-y-4 bg-white/5 backdrop-blur-md p-4 rounded-card-glass border border-white/10">
 <div className="grid grid-cols-2 gap-4">
 <div className="space-y-1">
 <p className="text-label text-white/40">Residual</p>
 <p className={cn("text-display-sm tracking-tighter tabular-nums text-white")}>
 {formatCurrency(remaining)}
 </p>
 </div>
 <div className="text-right space-y-1">
 <p className="text-label text-white/40">Burn Runway</p>
 ...
 <span className="text-label text-white/40">Safe Daily Quota</span>
 <span className="text-body-md text-white tabular-nums tracking-tighter">
