...
export const AddBudgetModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleNext = () => {
    ...
    if (step === 1 && !budgetName) {
      setErrors({ name: "Nama anggaran tidak boleh kosong." });
      return;
    }
  ...
  const handleSubmit = async () => {
    setErrors({});
    if (targetAmount <= 0) {
      setErrors({ amount: "Target anggaran harus lebih besar dari nol." });
      return;
    }
            ...
            >
              {step === 1 && (
                <div className="space-y-4 pt-2">
                  <Label htmlFor="budget-name" className={cn("text-label-md text-label text-muted-foreground ml-1", errors.name && "text-destructive")}>Nama Anggaran</Label>
                  <Input
                    id="budget-name"
                    ...
                    className={cn("h-12 rounded-card bg-muted border border-border/15", errors.name && "border-destructive")}
                    required autoFocus
                  />
                  {errors.name && <p className="text-label-md text-destructive ml-1">{errors.name}</p>}
                </div>
              )}
              {step === 2 && (
                <div className="space-y-4 pt-2">
              ...
              {((step === 3 && !hasSubCategories) || step === 4) && (
                <div className="space-y-6 pt-2">
                  ...
                  <div className="space-y-3 text-center">
                    <Label htmlFor="target-amount" className={cn("text-label-md text-label text-muted-foreground/40", errors.amount && "text-destructive")}>Target Allowance</Label>
                    <div className="flex flex-col items-center justify-center gap-2">
                      <Input
                        id="target-amount"
                        value={formatCurrency(targetAmount)}
                        onChange={(e) => {
                          setTargetAmount(parseInt(e.target.value.replace(/[^0-9]/g, '')) || 0);
                          if (errors.amount) setErrors(prev => ({ ...prev, amount: '' }));
                        }}
                        className={cn("text-display-lg font-medium border-none focus-visible:ring-0 text-center bg-transparent placeholder:text-muted-foreground/10 h-auto p-0 tracking-tighter", errors.amount && "text-destructive animate-shake")}
                        placeholder="Rp 0"
                        inputMode="numeric"
                        autoFocus
                      />
                      {errors.amount && <p className="text-label-md text-destructive">{errors.amount}</p>}
                    </div>
                  </div>
