...
const ShinyText = ({ text }: { text: string }) => (
 ...
 className="bg-[linear-gradient(110deg,hsl(var(--gray-400)/0.5),45%,hsl(var(--gray-50)),55%,hsl(var(--gray-400)/0.5))] bg-[length:200%_100%] bg-clip-text text-transparent dark:bg-[linear-gradient(110deg,hsl(var(--gray-600)),45%,hsl(var(--gray-50)),55%,hsl(var(--gray-600)))]"
 >
 {text}
 </motion.span>
...
export const AIChatDrawer = ({ isOpen, onClose }: AIChatDrawerProps) => {
 ...
 <div className={cn('px-4 py-3 rounded-2xl text-body-md leading-relaxed', message.role === 'user'? 'bg-primary text-primary-foreground': 'bg-card text-foreground prose prose-sm max-w-none prose-p:leading-relaxed prose-strong:prose-strong:text-foreground prose-a:text-primary dark:prose-invert')}>
 {message.role === 'user'? (
 displayText
 ) : isPending ? (
 ...
 className="rounded-full border border-border/60 bg-card px-3 py-1.5 text-label-md text-primary transition-colors hover:border-primary/35 hover:bg-primary/5"
 >
 ...
 whileTap={{ scale: 0.99 }}
 className="w-full rounded-2xl border border-border/45 bg-card/72 px-3 py-2.5 text-left text-label-md font-medium leading-relaxed text-muted-foreground shadow-elevation-1 backdrop-blur-sm transition-all hover:border-border/70 hover:bg-card hover:text-foreground"
 >
 {suggestion.value}
 </motion.button>
 ))}
 ...
 <div className="px-4 py-3 rounded-2xl bg-card text-foreground flex items-center gap-2 text-body-md">
 <ShinyText text="Lemon Coach sedang berpikir..."/>
 </div>
 </div>
 </div>
 ...
 <p className="ml-1 text-label text-muted-foreground/80">
 Saran Pertanyaan
 </p>
 ...
 exit={{ opacity: 0, y: -5 }}
 transition={{ duration: 0.2 }}
 className="pointer-events-none absolute inset-y-0 left-4 flex items-center text-body-md leading-none text-muted-foreground/60"
 >
