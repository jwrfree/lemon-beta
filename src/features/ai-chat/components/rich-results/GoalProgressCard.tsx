...
export const GoalProgressCard = ({ data }: GoalProgressCardProps) => {
 ...
 <div>
 <div className="text-body-md text-foreground">{data.name}</div>
 <p className="mt-1 text-label-md text-muted-foreground">
 {formatCurrency(data.current ?? 0)} dari {formatCurrency(data.target)}
 </p>
 </div>
 <div className={`inline-flex items-center rounded-full px-2.5 py-1 text-label-md ${statusStyles[status]}`}>
 {statusCopy[status]}
 </div>
 </div>
 ...
 <div className="mt-3 rounded-2xl bg-amber-50 px-3 py-2 text-body-md text-amber-900 dark:bg-amber-950 dark:text-amber-100">
